...
- `src/routes/shopping-lists/index.tsx` and `src/components/shopping-lists/overview-list.tsx` — same treatment for the concept list overview flow; keep the segmented tab header inside the sticky region so it follows the cards.
- `src/routes/shopping-lists/$listId.tsx`, `src/components/shopping-lists/concept-toolbar.tsx`, and `src/components/shopping-lists/ready/ready-toolbar.tsx` — remove the per-toolbar `sticky` positioning, move both toolbars into the shared sticky header, and ensure their instrumentation continues to fire while they inherit the shared elevation/backdrop styling.
...
2. **Lists: Parts, Shopping List Overview, Boxes, Types, Sellers**
   ...
   - Refactor the list component bodies:
     - Wrap the existing header (title + buttons, segmented tabs, filters) in the shared `StickyHeader` component with `top-0`, `z-20`, `bg-background/95`, and `backdrop-blur` to match the design in `docs/contribute/ui/patterns/part_management.md`. For `ShoppingListsOverview`, keep the segmented tab header and summary text inside this wrapper so it scrolls with the cards and adopts the shared stickiness.
...
3. **Shopping List detail view (Concept + Ready)**
   - In `src/routes/shopping-lists/$listId.tsx`, promote the top-level container for both the concept and ready views to `flex h-full flex-col overflow-hidden` and render the shared sticky header once, above the shared scroll region that hosts the table or seller groups.
   - Move `ConceptToolbar` and `ReadyToolbar` inside the new sticky wrapper and strip the `sticky`/`top-0` classes from both components so they rely on the shared infrastructure. Preserve their current instrumentation calls and data-testids.
   - Collocate `ConceptHeader` (and any tab-like or breadcrumb content) inside the sticky wrapper so the header, toolbars, and summary badges move together. Ensure the scrollable region beneath uses `flex-1 overflow-auto` and exposes deterministic selectors for Playwright.
   - Confirm the ready-state dialogs and overlays still restore focus after the header refactor, and document any invariants with short guidepost comments if layout interactions become non-obvious.

4. **Forms: Part creation, duplication, and edit**
...
5. **Playwright coverage & helpers**
   ...
   - Update shopping-list detail specs (concept + ready) to verify the shared sticky header emits `shoppingLists.detail.header` (or similar) events once the observer fires, and that the toolbars remain visible while the table or seller group list scrolls.
...
2. **Shopping list overview header sticks** — From `/shopping-lists`, scroll through a seeded list until the sentinel fires; assert `Create Concept List`, the search input, summary, and segmented tabs stay visible while the card grid scrolls.
3. **Shopping list detail header consolidates** — Exercise both Concept and Ready states on `/shopping-lists/:id`, scroll the table or seller group list, and confirm the shared header (breadcrumb, status badges, toolbar actions) emits the sticky event and never detaches from the viewport.
4. **Other resource lists inherit sticky behavior** — Repeat for `/boxes`, `/types`, and `/sellers`, ensuring their headers emit respective sticky events and that disabled primary buttons during loading/error still occupy the sticky slot.
5. **Part form sticky toolbar** — Load `/parts/new`, scroll to the bottom of the form, and confirm `Cancel` / `Add Part` persist in view. Trigger a validation error to ensure the sticky toolbar does not block toasts or error summaries.
6. **Edit + duplicate flows** — Enter edit mode from a part detail, scroll while the toolbar is stuck, submit changes, and verify instrumentation still reports `Form` events (`parts.form.editor`). Duplicate flow should exercise the progress indicator inside the scroll region.
7. **Keyboard/focus regression** — Tab through the sticky headers to confirm focus order remains logical; this can be a scripted Playwright step or manual fallback documented as part of the test plan.
...
2. **Phase 2 — Shopping list detail consolidation**: refactor the concept/ready views to use the shared sticky header, remove duplicated stickiness on the toolbars, and update tests to cover the combined header instrumentation.
3. **Phase 3 — Remaining resource lists**: adopt the shared component across boxes, types, sellers (and any other list using the same header pattern), adjusting tests in lockstep.
4. **Phase 4 — Form polish**: refactor `PartForm` (and other long forms if discovered) to the sticky toolbar layout, then extend Playwright coverage for edit and duplication flows.
