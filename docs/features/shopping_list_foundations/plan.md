...
- `src/hooks/use-shopping-lists.ts`: new hook module that wraps generated shopping list queries/mutations (`useGetShoppingLists`, `useGetShoppingListsByListId`, `usePostShoppingLists`, `usePutShoppingListsByListId`, `useDeleteShoppingListsByListId`, `useGetShoppingListsLinesByListId`, `usePostPartsShoppingListMembershipsByPartKey`, `usePutShoppingListLinesByLineId`, `useDeleteShoppingListLinesByLineId`, `usePutShoppingListsStatusByListId`) and maps snake_case payloads into camelCase domain models. We intentionally lean on the part-membership endpoint (`/api/parts/{part_key}/shopping-list-memberships`) for Concept line creation because it gives us the part context needed for duplicate prevention and aligns with backend recommendations.
...
3. Create `concept-line-form.tsx` to handle both “Add row” and “Edit line” flows:
   ...
   - Submit through the relevant mutation (`usePostPartsShoppingListMembershipsByPartKey` or `usePutShoppingListLinesByLineId`), catching `ApiError` 409 responses to display the duplicate warning (“if user tries to add same Part again, show error directing them to edit the existing line”).
